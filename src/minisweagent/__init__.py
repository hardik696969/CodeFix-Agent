...
# Check for a project-local .env in the current working directory
local_config_file = Path.cwd() / ".env"
...
if not os.getenv("MSWEA_SILENT_STARTUP"):
    ...
    if local_config_file.exists():
        Console().print(
            f"Loading local config from [bold cyan]'{local_config_file}'[/bold cyan] (overrides global)"
        )

dotenv.load_dotenv(dotenv_path=global_config_file)
# Local .env takes precedence — override=True ensures local values win over global ones
if local_config_file.exists():
